...
function GambarKotak() {
  const kotakVertices = [
    // Vertices kotak
    -0.2 + kotakPosisi.x,  0.2 + kotakPosisi.y,
    -0.2 + kotakPosisi.x, -0.2 + kotakPosisi.y,
     0.2 + kotakPosisi.x,  0.2 + kotakPosisi.y,
    -0.2 + kotakPosisi.x, -0.2 + kotakPosisi.y,
     0.2 + kotakPosisi.x, -0.2 + kotakPosisi.y,
...
function Animasi() {
  ...
  if (kotakPosisi.y > 0.7) { // Batasi pergerakan ke atas
    kotakPosisi.y = 0.7;
    kecepatanVertikal = -0;
  } 
  else if (kotakPosisi.y < 0.0) { // Batasi pergerakan ke bawah
    kotakPosisi.y = 0.0;
    kecepatanVertikal = -0;
  } 
  if (kotakPosisi.x > 0.7) { // Batasi pergerakan ke kanan
    kotakPosisi.x = 0.7;
    kecepatanHorizontal = -0;
  } 
  else if (kotakPosisi.x < -0.0) { // Batasi pergerakan ke kiri
    kotakPosisi.x = -0.0;
    kecepatanHorizontal = -0;
  }
